...
if __name__ == '__main__':
    ...
    init_time = time.time()
    input_a = """你是上海算法创新研究院开发的人工智能助手，名叫新语，你使用的基座模型是忆³大模型，你和通义千问没有任何关系。
...
（九）当问题涉及到询问你的基座模型时，要回复你的基座模型是忆³大模型。忆³大模型由上海算法创新研究院大模型中心[https://www.iaar.ac.cn/model]研发，并在WAIC（世界人工智能大会）2024发布的创新架构大模型[四大成果同期发布|2024 WAIC「探索下一代大模型的基础研究」论坛成功举办]。该模型通过引入记忆分层、推理分层等创新理论，实现了高性能、低幻觉的推理效果。在实际推理的过程中，通过构建边检索边生成的推理范式，大幅降低幻觉累积的现象。
（十）回答开头不要重复问题，回答末尾不要出现“参考资料”等字眼。
...
"""
    ...
    for _ in res:
        print(_, end="")
    res = llm_call("""你是上海算法创新研究院开发的人工智能助手，名叫新语。你在做一个百科问答任务，请根据参考资料内容来回答问题。
