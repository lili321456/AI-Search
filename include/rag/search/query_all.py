...
def query_xinhua_vector_db_docId(raw_text, body_input=None, topK = 20, docList=[]):
    """
        curl --location --request POST 'https://data.xinhua-news.cn/ChatSearch/search_news_emb/' \
        --header 'Accept: application/json;charset=gb2312' \
        --header 'Content-Type: text/plain' \
    ...
    """
    if len(docList) < 1:
        body = {'searchKeyWords': raw_text, 'embeddingName': 'embeddings_stella', 'limit': f'{topK}', "more_detail": "True"}
    else:
        body = {'searchKeyWords': raw_text, 'embeddingName': 'embeddings_stella','doc_list':docList, 'limit': f'{topK}', "more_detail": "True"}

    if 'embeddingName' in body_input and body_input['embeddingName'] is not None:
        body['embeddingName'] = body_input['embeddingName']
    if 'mediaTypeId' in body_input and body_input['mediaTypeId'] is not None:
        body['mediaTypeId'] = body_input['mediaTypeId']
    if 'more_detail' in body_input and body_input['more_detail'] is not None:
        if body_input['more_detail'] in ['True', 'False']:
            body['more_detail'] = body_input['more_detail']



    cidList = ["c0030006002", "c0030006001", "c0030015", "c0030002", "c0050001", "c0050004", "c0050020", "c0030173",
               "c0090159", "c0090178", "c0030404"]
    if 'searchCategoryId' in body_input and body_input['searchCategoryId'] is not None and len(
            body_input['searchCategoryId']) > 0:
        cidList = body_input['searchCategoryId']
    body['searchCategoryId'] = cidList

    tm_start = "1978-00-00 00:00:00"
    tm_end = "3224-00-00 00:00:00"
    if "start_time" in body_input and body_input['start_time'] is not None and len(body_input['start_time']) > 5:
        body['start_time'] = body_input['start_time'] + ' 00:00:00'
    if "end_time" in body_input and body_input['end_time'] is not None and len(body_input['end_time']) > 5:
        body['end_time'] = body_input['end_time'] + ' 23:59:59'
    # if 'required_keywords' in body_input and body_input['required_keywords'] is not None and len(body_input['required_keywords']) > 0:
    #     body['required_keywords'] = body_input['required_keywords']
    # if 'optional_keywords' in body_input and body_input['optional_keywords'] is not None and len(body_input['optional_keywords']) > 0:
    #     body['optional_keywords'] = body_input['optional_keywords']

    flag = 0
    #log.debug("milvus检索的body:{}".format(body))
    try:
        if CommonConfig.get_is_xinhua_env():
            # 发起POST请求
            res = requests.post(
                url=search_xinhua['vector_db_url'],
                json=body,
            ...
            #本地环境
            data = {}
            data.update({"request_id": "MLbqk37lgnX1oRrZAsQF7aV2gLmKA8NQsg19NxlEoMf6CIY",
                         "question": json.dumps({"body": body, "question": "question8888"})})
            url = "https://data.xinhua-news.cn/ChatSearch/query_reference_task_service/"
            headers = {
                "Content-Type": "application/json"
            }
            # 发送 POST 请求
            ...
            res =  json.loads(tt['xinyu']['err_msg'])['result']
        #log.debug("esMilvus:{}".format(res[0:1]))
    except Exception as e:
        import traceback
        print(traceback.format_exc())
        error = f"query_credible error {e}"
        ...
        res = f"query_credible_error: {error}"
        flag = 1
    return res, SEARCH_TYPE_XINHUA_ES_MILVUS, flag
...
def query_xinhua_search_news_full_text(raw_text, body_input, topK = 20):
    """
        es 全文检索
        curl --location --request POST 'https://data.xinhua-news.cn/ChatSearch/search_news_es/' \
        --header 'Accept: application/json;charset=gb2312' \
    ...
    cidList = ["c0030006002", "c0030006001", "c0030015", "c0030002", "c0050001", "c0050004", "c0050020", "c0030173",
               "c0090159", "c0090178", "c0030404"]
    if "start_time" in body_input and body_input['start_time'] is not None and len(body_input['start_time']) > 5:
        tm_start = body_input['start_time'] + ' 00:00:00'
    if "end_time" in body_input and body_input['end_time'] is not None and len(body_input['end_time']) > 5:
        tm_end = body_input['end_time'] + ' 23:59:59'
    if 'searchCategoryId' in body_input and body_input['searchCategoryId'] is not None and len(body_input['searchCategoryId']) > 0:
        cidList = body_input['searchCategoryId']

    dsl_w_time = {"dsl":{"query": {"bool": {
        "must": [{"match": {"content": f"{raw_text}"}},
                 {"range": {"broadcastDate": {"gte": tm_start, "lt": tm_end}}},
                 {"terms": {"searchCategoryId": cidList}}]}},
        "size": topK}}
    dsl_o_time = {"dsl":{"query": {"bool": {
        "must": [{"match": {"content": f"{raw_text}"}},
                 {"terms": {"searchCategoryId": cidList}}]}},
        "size": topK}}
    ...
    if "required_keywords" in body_input and body_input['required_keywords'] is not None and len(body_input['required_keywords']) > 0:
        for keyword in body_input['required_keywords']:
            body['dsl']['query']['bool']['must'].append({"match_phrase": {"content": keyword}})

    if "optional_keywords" in body_input and body_input['optional_keywords'] is not None and len(body_input['optional_keywords']) > 0:
        tmp = {"bool": {"should": []}}
        for keyword in body_input['optional_keywords']:
            tmp['bool']['should'].append({"match_phrase": {"content": keyword}})
        body['dsl']['query']['bool']['must'].append(tmp)

    #log.debug("ES检索的body:{}".format(body))
    try:
        if CommonConfig.get_is_xinhua_env():
            # 发起POST请求
            res = requests.post(
            ...
            # 本地环境
            data= {}
            data.update({"request_id": "LBvYzIlPjAf5PCi8FmkhEfgC1a2DjY2Ef49S7jqFPpa0ot9",
                         "question": json.dumps({"body": body, "question": "question8888"})})
            url = "https://data.xinhua-news.cn/ChatSearch/query_reference_task_service/"
            headers = {
                "Content-Type": "application/json"
...
def query_xinhua_vector_inner(raw_text, body_input=None):
    '''直接通过内部接口获取新华社检索结果'''
    log.debug("query_xinhua_vector_inner, query:{}, body:{}".format(raw_text, body_input))
    body = {"question": raw_text,
            "chatRecord": [],
            "referAnswer": "",
            "sourceString": "",
            "referContent": "",
    ...
    if 'start_time' in body_input and len(body_input['start_time']) > 5:
        body['time'] = body_input['start_time'] + '/' + body_input['end_time']
    if 'optional_keywords' in body_input:
        body['keywordOne'] = ','.join(body_input['optional_keywords'])
    if 'required_keywords' in body_input:
        body['keywordAll'] = ','.join(body_input['required_keywords'])
    if 'searchCategoryId' in body_input:
        body['cid'] = ','.join(body_input['searchCategoryId'])
    if 'mediaTypeId' in body_input:
        body['mediaTypeId'] = body_input['mediaTypeId'][0]
    ...
    try:
        if CommonConfig.get_is_xinhua_env():
            ...
            #本地环境
            data = {}
            data.update({"request_id": "GxAUWhYAGKqz^iQdYr%EDgp5WGAhK6^Fb8%KHWuo1E7I",
                         "question": json.dumps({"body": body, "question": "question8888"})})
            url = "https://data.xinhua-news.cn/ChatSearch/query_reference_task_service/"
            headers = {
                "Content-Type": "application/json"
    ...
    return res, SEARCH_TYPE_XINHUA_INNER, 0


def query_xinhua_vector_db_ES(raw_text, body=None, useEs=False, rerank=True, LLMfilter=False, keyWordFilter=True, rerankModel=None, **kwargs):
    # {
    #     "embeddingName": "embeddings_stella",      milvus
    #     "more_detail": "False",                     milvus
    #     "mediaTypeId": ["Photo"],                   milvus
    ...
    # }
    # es 返回数量
    log.debug("query_xinhua_vector_db_ES, query:{}, body:{}".format(raw_text,body))
    topk_es = 1000 # 默认值30
    if 'topk_es' in rerank_config:
        topk_es = rerank_config['topk_es']
    # milvus 返回数量
    topk_vec = 500
    if 'topk_vec' in rerank_config:
        topk_vec = rerank_config['topk_vec']

    # rerank 返回数量
    topk_rerank = 100
    if 'topk_rerank' in rerank_config:
        topk_rerank = rerank_config['topk_rerank']

    # ES 检索
    docList = []
    if useEs:
        res_es = query_xinhua_search_news_full_text(raw_text, body, topK=topk_es)
        docList = parse_docId(res_es)
        log.info("es docList length:{}".format(len(docList)))

    # 向量检索
    res0, b, c = query_xinhua_vector_db_docId(raw_text, body_input=body, topK=topk_vec, docList=docList)
    log.info("milvus docList length:{}".format(len(res0)))

    # 向量距离过滤
    # if True:
    #     tmp_res = []
    #     for idx, rr in enumerate(res0):
    #         if rr['distance'] > 0.72:
    #             tmp_res.append(rr)
    #     res0 = tmp_res
    # log.info("向量距离过滤 docList length:{}".format(len(res0)))

    # 关键词过滤 required_keywords
    if (keyWordFilter
            and "required_keywords" in body
            and body['required_keywords'] is not None
            and len(body['required_keywords']) > 0):

        resNew1 = []
        for idx, rr in enumerate(res0):
            content = rr['chunkContent']
            valid = True
            for keyword in body['required_keywords']:
                if keyword not in content:
                    valid = False
                    break
            if valid:
                resNew1.append(res0[idx])
        res0 = resNew1

    # 关键词过滤 optional_keywords
    if (keyWordFilter
            and "optional_keywords" in body
            and body['optional_keywords'] is not None
            and len(body['optional_keywords']) > 0):

        res_new1 = []
        for idx, rr in enumerate(res0):
            content = rr['chunkContent']
            valid = False
            for keyword in body['optional_keywords']:
                if keyword in content:
                    valid = True
                    break
            if valid:
                res_new1.append(res0[idx])
        res0 = res_new1
    ...
    if rerank:
        docs = []
        for idx, rr in enumerate(res0):
            docs.append(rr['chunkContent'])
        ans_rerank = rerankBge(raw_text, docs, rerank_config, rerankModel=rerankModel)
        if ans_rerank is not None:
            indexes = top_n_indexes(ans_rerank, topk_rerank)
        else:
            indexes = []
        res_tmp = []
        for i in indexes:
            res0[i]['rerank_score'] = ans_rerank[i]
            res_tmp.append(res0[i])
        res0 = res_tmp
        log.info("rerank docList length:{}".format(len(res_tmp)))
    else:
        res0 = res0[0:topk_rerank]
    ...
    if LLMfilter:
        ...
        contentList = []
        for rr in res0:
            contentList.append(rr['chunkContent'])

        ll = len(contentList)
        queryList = [raw_text]*ll
        vec_sim_with_query_llm, resList = batchQueryDocCorrelationSeq(queryList, contentList)
        #log.debug("resList:{}".format(resList))
        #log.debug("len res0:{}".format(len(res0)))
        #log.debug("len resList:{}".format(len(resList)))

        resNew = []
        reject_ref = []
        for idx, bool_llm in enumerate(resList):
            if bool_llm:
                resNew.append(res0[idx])
            else:
                reject_ref.append(res0[idx])

        while len(resNew)<3:
            resNew.append(reject_ref[0])
            del reject_ref[0]
    ...
    #return query_xinhua_vector_db_docId(raw_text, body=query_kwargs, docList=docList)
